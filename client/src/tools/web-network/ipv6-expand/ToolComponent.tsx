  ...
  <SimpleTool
    ...
    execute={(input: string) => {
      ...
      return [
        `展开格式: ${expanded}`,
        `压缩格式: ${compressed}`,
        `地址类型: ${expanded.startsWith('fe80') ? '链路本地' : expanded.startsWith('fc') || expanded.startsWith('fd') ? '唯一本地' : expanded.startsWith('ff') ? '组播' : expanded.startsWith('2001:db8') ? '文档示例' : '全局单播'}`,
      ].join('\n');
    }}
  />
);
